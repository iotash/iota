...
pub const SKILLS_CATALOG_INSTRUCTION: &str = "To use a skill, call the load_skill tool with the skill's name and follow the instructions it returns; read files the skill references by calling load_skill again with the \"file\" argument, and run its bundled scripts with the bash tool.";
