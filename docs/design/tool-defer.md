...
| `reference` | shipped (live-verified: official Anthropic + claude-sonnet-4-5 — the server-side regex search runs and expands references in-response; note Kimi's anthropic-COMPAT endpoint silently ignores `defer_loading`, deferring there simply advertises nothing) | Anthropic `defer_loading`: deferred tools carry the flag plus the server-side `tool_search_tool_regex` entry; search + `tool_reference` expansion happen WITHIN a response, so search-and-call completes in one round with no client round trip (anthropic dialect, Claude 4.5+). The server search blocks (server_tool_use + tool_search_tool_result) are captured into RawContent and replayed at the front of their assistant message on later rounds — the replay keeps the schema expansion alive (without it the model blind-calls discovered tools with GUESSED argument names: a correctness failure, live-verified with an unguessable schema). Capture and replay are gated on the request carrying defer_loading tools — the blocks belong to the reference protocol, so the gate is exact (no URL heuristics, no probing): a proxied official endpoint running reference replays; a resumed session on a provider whose requests never carry defer_loading strips automatically (compat endpoints like Kimi hard-400 on the block types). Blocks persist via the session blob and survive resume |
