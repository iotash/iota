...
- Cargo features: **none** that affect the binary. The only `[features]` entry is `testing` (the shared test fakes), enabled for tests by the self-dev-dependency. ratatui's `scrolling-regions` is off since 2026-09-25: the inline terminal is iota's own (DIVERGENCES X-54).
...
| `shell/{mod,exec,interp,jobs,selfcall}.rs`, `shell/sandbox/{mod,darwin,linux,other}.rs` | process execution, the MECHANISM under the `shell` tool (its pins are §5): the child in its process group, the capped output pipe, the interpreter choice, the background-job registry, the Seatbelt/bwrap sandboxes — and in `selfcall` the one exception to them, the rule that a command line whose first word is the running binary is iota itself (X-44) | `provider` and below |
| `agents/{mod,skills,harness}.rs` | the AGENTS.md overlay (root discovery, the chain, the three-segment `compose_send_history`: harness, `<instructions>`-wrapped user prompt, overlay), skills discovery, and in `harness` the built-in harness prompt — the preamble, the `<environment>` block over a caller-filled `Environment`, `<iota_cli>` for the shell set — composed at send time and never stored (X-43) | `provider` and below |
...
| `mcp/{mod,config,manager,transport,auth,error}.rs` | the rmcp manager: server config and the `--mcp` flag, the transports, the 30 s connect fan-out with its config-order merge, `mcp__<segment>__<tool>` naming, the live tool view, routing and close; `auth` the OAuth 2.1 side — the file token store, the login/logout flows, the runtime manager the HTTP transport is wrapped in for a server that logs in (`auth: oauth`, or the default `auto` once a token file exists) | `tool` and below |
...
| `host/{mod,ansi,herdr,cmux,background}.rs` | host integration: the presenter's per-capability fan-out (progress, attention ping, session identity, exit clean-up) over the innermost detected host plus the ANSI fallback (`DETECTORS` runs inner to outer and the first match is kept — X-47), and the one capability gathered from every host, the facts it adds to the harness prompt's `<environment>` (`host: herdr`, `herdr pane: <id>`); the ANSI host (OSC 9 / 9;4 through the facade), the herdr host (the pane's lifecycle, session and release as JSON lines over herdr's socket, or its CLI where there is no Unix socket, from one worker over a bounded queue), the cmux host, the background probe | `ui` and below |
| `repl/{mod,run,state,liveparams,catalog,editpicker,errors,title,systemtab}.rs`, `repl/turn/{mod,tools,retry,phases,steer,interrupt,approval,interact}.rs`, `repl/render/{mod,transcript,group,uisink,styles,diff,banner,replay,mcpreport}.rs`, `repl/context/{mod,meter,tokens}.rs`, `repl/commands/…` | the interactive loop over the facade: `run.rs` the loop, `state.rs` its state as `Conversation`/`SessionSlot`/`UiHandles`, `turn/` one turn (`TurnEngine` with the turn-level retry, the tool walk, steering, interrupts, approvals), `render/` what it draws, `context/` the token accounting, `catalog.rs` the `/model` listing, `liveparams.rs` the live half of the layered parameters, `commands/` the slash commands (no `/mcp`, X-42: the MCP tab of `tools.rs` is the chat's view of the servers) | everything below |
...
- `Tool` / `Dispatcher` with default capability methods; ownership is one lookup through the `Owner` trait (the registry, the MCP manager and the live union each vouch for what they own; `prefix_of` reads a table); `search_tools() -> Option<Vec<ToolDef>>` (the union forwards to the FIRST part that HAS the capability); `supports_parallel(name, Option<&JsonObject>)` and `requires_approval(name, Option<&JsonObject>)` per CALL (`None` = the argument-less probe) — the `shell` tool's answers to both depend on the command line (X-44).
- `RunCtx { cancel, budget, artifact }` (`tool::context`) cloned into every tool call.
- `ToolEnv { project_root: Option<PathBuf>, dirs: HostDirs, interactor: Option<Arc<dyn Interactor>> }`; `root()` = project_root else `dirs.cwd` else `current_dir()`, then `std::path::absolute`. `HostDirs` carries the canonical executable (`exe`, `app::canonical` — the `\\?\` verbatim prefix stripped on Windows) beside the directories: the one process fact the `shell` set's self-invocation rule and the harness prompt both read, resolved once at the edge.
...
- Approval/parallel: the REFUSAL lives in `headless::QuietHost`; tools only report. `glob`/`grep`/`list_dir`/`read_file` always run in parallel, and so does `shell` (DIVERGENCES X-05: the model wrote both command lines) — except a sandboxed set's iota call that must be asked about, which takes the serial path because the batch has no gate (X-44).
...
- The `shell` MECHANISM (`src/shell/`, the layer the `shell` tool is a policy over) is pinned: one interpreter child per call, in its own process group on Unix and a Job Object on Windows; ONE combined output pipe (`std::io::pipe()` — the same `pipe(2)`, portable and `O_CLOEXEC`) read under the byte cap; an explicit `PWD` on Unix and none on Windows; kill = `killpg(SIGKILL)` with ESRCH ignored / `TerminateJobObject`, then a 3 s wait for the pipe to drain (`timeout(3s, read_to_end)`); the result classified in one fixed order (timeout, signal, exit status). WHICH interpreter runs is `shell::interp`'s one pure answer over an injected machine — `bash -c` on Unix, on Windows the first of Git Bash, PowerShell and `cmd.exe` present (DIVERGENCES X-17) — and a sandbox binary appearing or disappearing later never changes a running set. ONE command line leaves the sandbox: iota itself — `shell::selfcall::is_self_invocation` reads the first word with POSIX quoting, resolves it (a path as written, a bare name on `PATH`) and compares the canonical file to `HostDirs::exe`; a pipe, chain, newline or command substitution outside single quotes keeps the line in (redirections do not). The `shell` tool then spawns with `sandbox: None`, nothing else changed: approval as an unsandboxed set would ask it (`auto_run` waives), the header and prompt marked `(outside the sandbox)`, no parallel batch for a call that must be asked about (X-44).
...
- OAuth 2.1 (`mcp::auth`, brain page `mcp-cli-and-oauth`; rmcp's `auth` feature, whose `oauth2` 5.0.0 is why `deny.toml` skips eleven older crate versions). `auth` is three-state (`mcp::config::AuthMode`): not written is `auto` — discovered, as Claude Code and Codex do it — `oauth` forces the login, `none` forbids it, and `auto` with an `Authorization` header of the entry's own is `none` (`ServerConfig::effective_auth`: the credential written is the one to fix). `transport::connect_one` sends an `auto` server through the OAuth transport when a token file exists for its name, else bare, where a 401/403 at the handshake (`is_auth_required`: a challenge, `insufficient_scope`, or a bare status rmcp reports as `HTTP 401 …`) is `McpError::NotLoggedIn` and sets `ServerStatus::login_required`, which is what gives an `auto` server a `login` state in `Manager::servers()` (`iota mcp list --probe`'s `needs login`) before it has a token file; `none` is bare with every failure a `Connect`. A server that logs in connects through `AuthClient<reqwest::Client>` over an `AuthorizationManager` built from the token file alone — `~/.iota/mcp/auth/<name>.json`, mode 0600, holding rmcp's `StoredCredentials` beside the endpoint and the authorization-server metadata, so a refresh needs no discovery. No file, or a handshake the server answers 401 after the refresh failed too, is `McpError::NotLoggedIn` (`not logged in: run iota mcp login <name>`): that one server is `Failed`, the run goes on. `login` — any HTTP entry that is not an effective `none` (`ServerConfig::login_refusal` names why a stdio server, `auth: none` or an own `Authorization` header is refused) — = discovery (a server that neither challenges nor publishes protected-resource metadata is `LoginError::NoMetadata`, "does not ask for a login", not a login against rmcp's synthesised legacy endpoints) → the client's identity in order (the entry's `client_id` with its `${env:VAR}` secret, else dynamic registration, else the Client ID Metadata Document `mcp::auth::CLIENT_METADATA_URL` when the server accepts one, else the error that names the ways out) → the resource's scopes (its 401 challenge, its RFC 9728 metadata; `offline_access` when the server lists it; none when the resource names none) with the RFC 8707 `resource` → the URL to `$BROWSER` / the platform opener / stdout → a loopback listener on `127.0.0.1:<port>/callback` — a random port, or for a pre-registered client the FIXED one its redirect URI was registered with (`redirect_port`, else `DEFAULT_REDIRECT_PORT` 17801; taken = `LoginError::PortInUse`) — with a pasted redirect URL and a 5-minute deadline beside it → the exchange writes the store; `logout` revokes (RFC 7009, best effort) and removes the file. The steps are the manager's, one call each — rmcp's `AuthorizationSession` orders the document before registration and picks the scopes itself. `Manager::login`/`logout`/`reconnect` are the in-session entries — handed to the REPL as `McpHooks::manager`, called by no slash command since `/mcp` left (X-42), waiting for the config toolset (brain page `config-tools`) to log a server in from inside a chat: a reconnect detaches the old session, its tools, segment and prefix (`State::session_of`) and merges the new result like any connect. `iota mcp add --url` follows the entry it wrote up (`cmd::mcp_cmd::FollowUp`): a probe of the endpoint — `mcp::auth::probe`, one bare `initialize` POST with a 10 s deadline and three answers, 401/403 `NeedsLogin` (the challenge parsed), 2xx `Open`, anything else `Unknown(why)` — and on `NeedsLogin` the flow of `iota mcp login` then and there (`login_to`, over the entry in hand rather than a second read of the files); `--auth oauth` or a `client_id` skips the probe and logs in at once, an effective `none` and `--no-login` skip both, `--no-browser` goes through to the login, and a login that does not finish is `RunError::McpAddLogin` — the entry stays, `Retry with: iota mcp login <name>`, exit 1 (X-41).
...
- `agents.<name>.model` is the model a run starts on and `agents.<name>.choices` what `/model` and `-M` pick from (X-48). `Config::choices_of` supplies the default for the second — every `models:` entry in DECLARATION order, which is why `ConfigFile` decodes that map as an ordered list and `Config` keeps `model_order` beside its name-ordered `BTreeMap` — and `Config::resolve_agent` hands out `Resolved.agent.choices` already defaulted, so the `/model` catalog, `-M` and the listings read one field. A wildcard under `model:` is refused at validation; a `model:` the choices do not cover is a load-time warning, the same rule `-M` follows.
...
- **The TUI's direct dependencies:**
  ...
  - `ratatui` (=0.30.2) — `Buffer`, `Buffer::diff`, `Paragraph`/`Line`/`Span` and the W6 `line_count` law (feature `unstable-rendered-line-info`). Its `Terminal` is NOT used (2026-09-25, DIVERGENCES X-54): `src/ui/runtime/inline_term.rs` owns the inline viewport — a plain field — and writes every byte counted from the cursor (no `MoveTo` after the startup anchor, no DECSTBM), the shape of bubbletea's `cursedRenderer` and Codex's `custom_terminal`. That retired W1 (recreation on a height change), W2 (the self-tracked top), W3 (the clear after recreation), W9 (partial-region scrolling; `scrolling-regions` is off) and the acknowledged size.
    **Upgrade checklist:** a ratatui upgrade touches only the buffer, its diff (the covered-cell rule for wide graphemes: `vt100_tests::wide_runes_insert_intact`) and the widgets; ratatui's inline-resize fixes (#2670, #2485, #984/#1964) no longer concern iota. The frozen resize protocol is X-52.
