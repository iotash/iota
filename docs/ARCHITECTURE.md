...
| G4 | `Env { project_root: Option<PathBuf>, .. }`, `Env::root() -> io::Result<PathBuf>` | **adopted** (+ `dirs: HostDirs` field, see G5) | CONTRACTS §2.6 |
| G5 | inject the environment (home/cwd/env vars) instead of reading it: `Config::load(.., &HostDirs, &dyn VarResolver, ..)`, `skill_roots(root, home)`, `resolve_run(.., &dyn EnvSource, ..)`, `vars::expand(s, &dyn VarResolver)` | **adopted and extended**: one `iota_core::app::HostDirs` value (home, cwd, temp, cache) is built once in `main` and threaded everywhere; **no test mutates process env**, so `serial_test`/`temp_env` are not needed at all | CONTRACTS §2.9, §2.10 |
...
| G7 | shell mechanism pinned: `process_group(0)`, ONE pipe via `nix::unistd::pipe()` + `try_clone`, `tokio::net::unix::pipe::Receiver::from_owned_fd`, explicit `PWD`, `killpg(SIGKILL)` ESRCH-ignored, 3 s WaitDelay as `timeout(3s, read_to_end)`, classification order shell.go:97-121 | **adopted** | CONTRACTS §4.7 |
...
| G23 | `parse_window_size` ported as a pure function (config parity) | **adopted** — `iota::window` | CONTRACTS §7.4 |
...
| `app.rs` | internal/app | `HostDirs`, the well-known names/dirs |
| `vars.rs` | internal/vars | `${var}` expansion, `VarResolver`, `EnvSource`/`ProcessEnv` (the one process-environment seam) |
| `paths.rs` | (filepath helpers) | `clean`, `rel`, `to_slash` |
...
| `tool/{ask,agent,delegate}.rs`, `tool/shell.rs`, `tool/code/{mod,tools,walk,udiff}.rs` | tool/ask.go, agent.go, delegate.go, shell.go, code.go | the five built-in sets (`shell.rs` = the `bash` tool's POLICY layer) |
| `shell/{mod,exec,sandbox_darwin,sandbox_linux,sandbox_other}.rs` | internal/shell | process execution + sandboxes (the MECHANISM layer) |
| `agents/{mod,skills}.rs` | internal/agents | `Overlay`, `compose_send_history`, skills |
| `mathtext/{mod,delim,parse,symbols,macros,inline,pict,layout}.rs` | internal/mathtext | the LaTeX math engine (T3, WP61/WP62): inline Unicode approximation, 2D layout (Go `Box` → `Pict`), the delimiter scanners; the sole `MathRenderer` impl behind the markdown hooks |
...
- **The crate-boundary hacks died with the boundaries**: the `impl_tunable_via_core!` macro (orphan rule)
  ...
  `EnvSource` live where they are used; no `#[path]`-mounted test remains (those became unit tests).
...
- `nix = { default-features = false, features = ["signal", "process", "fs"] }` (`fs` gates `unistd::pipe`).
...
- `#![forbid(unsafe_code)]` in `lib.rs` and `main.rs`; no `unsafe` module exists at all (`nix` covers pipes/signals; `std::os::unix::process::CommandExt::process_group` covers process groups).
...
- **TUI slice direct-dependency additions (phase 3, WP40; TUI_CONTRACTS §1.1/§12):**
  ...
  - `tiktoken-rs` (=0.12.0, WP53) — the `o200k_base` token counter behind the live context meter, `/compact` and the auto-compaction offer. Chosen for the property Go bought with `tiktoken-go` + `tiktoken-go-loader`: it BUNDLES the rank table (`include_str!` of the 3.6 MB `.tiktoken` asset), so counting a conversation is never a network call and never a runtime download. The encoder is built once behind a `OnceLock` on first use, and a build failure degrades to Go's `len(text)/4` byte heuristic rather than failing the chat. It brings `fancy-regex`, `bit-set`/`bit-vec`, `lazy_static` and `rustc-hash` transitively (`anyhow`, `base64`, `bstr` and `regex` are already in the graph).
...
6. **Cancellation completeness** — every long primitive selects on the token; `spawn_blocking` walks are capped; assert_cmd test sends SIGINT to a wiremock-stalled run and checks exit 130 + JSON `"error": "interrupted"`.
