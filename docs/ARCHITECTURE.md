...
- `assemble::build_mcp_configs` / `build_dispatcher` use only `mcp::config` types and take the MCP part as `Option<(Arc<dyn Dispatcher>, PrefixOf)>` (`None` = no server configured); `Manager` is named only in `cmd::run` and `interactive/mod.rs`.
