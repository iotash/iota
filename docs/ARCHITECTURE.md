...
- **The TUI's direct dependencies:**
  ...
  - `ratatui` (=0.30.2) — spike-validated `Viewport::Inline` + `insert_before` scrollback engine; feature `unstable-rendered-line-info` for the W6 `line_count` law, `scrolling-regions` always on (wart W9 is a per-emulator verification item in `docs/TUI-VERIFY.md` §2, not a build variant).
    **Upgrade checklist (2026-09-24):** the next ratatui release carries #2670 (an inline narrowing no longer does `ESC[2J` + `y = 0`) and #2485 (no cursor query during a resize). The byte pins must pass UNCHANGED — `vt100_tests`' no-`ESC[2J` / no-home-erase / at-most-one-band-scroll assertions and the resize byte sequence — and the acknowledged size (`LoopBackend::size`, W5; pinned by `term::tests::the_backend_answers_the_acknowledged_size_*`) must NOT be removed because upstream "fixed" the inline resize: #2670 lands at our first round's level (a CPR anchor and the old offset), not at W5's floor/band/overhang. The frozen resize protocol is X-52.
