...
| `app/env.rs` | internal/vars | `${var}` expansion, `VarResolver`, `EnvSource`/`ProcessEnv` (the one process-environment seam) |
...
| `app/env.rs` | internal/vars/vars.go, cmd/root.go (os.Getenv seam) | `VarResolver`, `expand`, `EnvSource`, `ProcessEnv` |
