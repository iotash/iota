...
| `repl/{mod,run,liveparams,errors,title,systemtab,commands/…}.rs`, `repl/turn/{mod,tools,retry,phases,steer,interrupt,approval,interact}.rs`, `repl/render/{mod,transcript,group,uisink,styles,diff,banner,replay,mcpreport}.rs`, `repl/context/{mod,meter,tokens}.rs` | chat/run.go and friends | the interactive loop over the facade: `run.rs` is the loop, `turn/` one turn, `render/` what it draws, `context/` the token accounting, `liveparams.rs` the chat's live half of the layered parameters |
