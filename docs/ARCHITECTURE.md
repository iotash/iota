...
| `tool/{ask,agent,delegate}.rs`, `tool/shell.rs`, `tool/code/{mod,tools,walk,udiff}.rs` | tool/ask.go, agent.go, delegate.go, shell.go, code.go | the five built-in sets (`shell.rs` = the `bash` tool's POLICY layer; its description follows the interpreter that will run the call, DIVERGENCES X-18) |
