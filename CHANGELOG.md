...
- **A terminal that doesn't answer iota's cursor query no longer loses a line
  ...
  now erases nothing above the prompt it can't locate. Resizing in tmux also no
  longer erases a line that tmux moves back onto the screen in the middle of a
  resize. Dragging the height while a reply streams in can still lose a line
  in tmux; that fix is in progress (X-52).
